...
Run `npm run db:deploy` against the intended production database and verify migration status as an explicit pre-promotion gate. The repository's `npm run build` only generates Prisma Client and compiles Next.js, and root `vercel.json` pins Vercel to that schema-neutral command. Confirm every reviewed deployment log uses `npm run build` followed by `prisma generate && next build`; do not promote if a log shows `prisma db push`, a migration command, or `--accept-data-loss`. Complete all regression gates before merging a reviewed feature branch into `main`, and do not rewrite DNS until the whole branch has final approval. See [`docs/INSTALL.md`](docs/INSTALL.md) for the exact release sequence.
