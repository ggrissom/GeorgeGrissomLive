...
Run `npm run db:deploy` against the intended production database and verify migration status as an explicit pre-promotion gate. The repository's `npm run build` only generates Prisma Client and compiles Next.js. Vercel must have no mutating Build Command override: set it to `npm run build` or allow package-script detection, then confirm the deployment log uses `prisma generate && next build`. Do not promote while a Vercel log still shows `prisma db push` or `--accept-data-loss`. Complete all regression gates before merging a reviewed feature branch into `main`, and do not rewrite DNS until the whole branch has final approval. See [`docs/INSTALL.md`](docs/INSTALL.md) for the exact release sequence.
